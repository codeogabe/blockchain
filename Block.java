...
public class Block {
    ...
    public synchronized String computeMerkleRoot(ArrayList<String> sList) {

        String root;

        //Puts user-input-list into an array
        String[] arr = sList.toArray(new String[0]);

        //Establishes length of list
        int sListLen = arr.length;

        //Creates double that equals, Log base2(length of list)
        Double dListSR = log2(sListLen);

        //Turns double into string
        String sListSR = dListSR.toString();
        String[] sListSRChars = sListSR.split("");

        //Determines whether length of list is a root of 2
        //ex1. log base2(8) = 3.0         ---> its a root of 2
        //ex2. log base2(9) = 3.169925001 ---> not a root of 2

        ArrayList<MerkleNode> mList = new ArrayList<>();

        if (sListSRChars[sListSRChars.length - 2].compareTo(".") == 0 && sListSRChars[sListSRChars.length - 1].compareTo("0") == 0) {

            //Creates arraylist of nodes needed to obtain root
            for (int x=0 ;x<sListLen; x++) {
                mList.add(new MerkleNode());
                mList.add(new MerkleNode());
            }
            mList.remove(mList.size()-1);

            //Generates hash for each item in sList and assigns it a node in mList
            BlockchainUtil oUtil = new BlockchainUtil();
            for (int x = 0; x < sList.size();x++){
                mList.get(x).sHash = oUtil.generateHash(sList.get(x));
            }

            //Creates a tree
            for (int x = sList.size(); x < mList.size();x++) {
                populateMerkleNode(mList.get(x), mList.get(x - sList.size() + (x - sList.size())), mList.get(x - sList.size() + 1 + (x - sList.size())));
            }
            root = mList.get(mList.size()).sHash;
        } else {
            System.out.println("List size isn't a power of 2");
            System.exit(0);

        }
        return mList.get(mList.size()).sHash;
    }

    public Double log2(int num) {

        //Math equivalent of log base2()
    ...
    private void populateMerkleNode(MerkleNode oNode, MerkleNode oLeftNode, MerkleNode oRightNode){
        ...
        oNode.sHash = outil.generateHash(outil.generateHash(oNode.oLeft.sHash) + outil.generateHash(oNode.oRight.sHash));
    }
